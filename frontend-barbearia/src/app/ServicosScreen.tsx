...
import { criarServico, listarServicos, ServiceResponse } from '../services/servicosService';
import {
  View,
  Text,
  TextInput,
...
export default function ServicosScreen() {
  ...
  // ─── ID do estabelecimento vindo da sessão ─────────────────────────────────
  // TODO: substituir pela leitura real do contexto de autenticação
  const ESTABLISHMENT_ID = '3fa85f64-5717-4562-b3fc-2c963f66afa6';
  ...
  async function salvar() {
    ...
    if (tela === 'editar' && servicoSelecionado) {
      // Edição: apenas atualiza estado local por enquanto
      setServicos(prev =>
        prev.map(s =>
          s.id === servicoSelecionado.id
            ? { ...s, nome: formNome, preco: formPreco, comissao: Number(formComissao) || s.comissao, duracao: formDuracao }
            : s,
        ),
      );
      setTela('lista');
    ...
    // ── Criação: chama o backend e recarrega a lista ──
    try {
      setSalvando(true);
      ...
      Alert.alert('Erro ao criar serviço', erro.message ?? 'Tente novamente.');
    } finally {
      setSalvando(false);
    }
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {isEditar && (
          <TouchableOpacity style={styles.btnExcluir} onPress={excluir} activeOpacity={0.85}>
